...
```{r}
## Completar aqui"
GradosTelecoSimple  <- read.table("./data/notamediagradosETSIT-simple.csv", sep = ",", header = TRUE)
```
...
y  <- as.matrix(GradosTelecoSimple$notamedia)
```
...
gradJ <- function(theta)
  {
  gradJ<- 2/length(y) * t(X) %*% ( X%*%theta  - y)
      ...
      if (j==2)
       {
        alpha=0.003
      theta <- theta - alpha * gradJ(theta)
      monitor[i+3000,]=c(i, theta, J(theta), gradJ(theta), alpha)
        }
      if (j==3)
       {
        alpha=0.01
      theta <- theta - alpha * gradJ(theta)
      monitor[i+6000,]=c(i, theta, J(theta), gradJ(theta), alpha)
        }
    }
  }
```
...
```{r}
## Completar aquí:
notamediagrados.lm <- lm(formula = notamedia ~ PAU_CALIFICACION_NOTA_MATERIA, data = GradosTelecoSimple)
